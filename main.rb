module Enumerable
  ...
  def my_select
    return to_enum unless block_given?
    select = Array.new
    self.my_each{|x| select.push(x) if yield(x)}
    select
  end
  ...
  def my_all?(args=nil)
    if !args.nil? # if arguments or method parameters were given 
      if args.is_a?(Class)
        self.my_each{|i| return false unless i.is_a?(args)}
      elsif args.is_a?(Regexp)
        self.my_each{|i| return false unless arg.match(i.to_s)}
      else
        self.my_each{ |i| return false unless i == args}
      end
    elsif block_given? #if block was given
      self.my_each{|i| false unless yield(x)}
    else
      self.my_each{|i| false unless i}
    end
    true
  end
  ...
  def my_any?(args=nil)
    if !args.nil? # if arguments or method parameters were given 
      if args.is_a?(Class)
        self.my_each{|i| return true if i.is_a?(args)}
      elsif args.is_a?(Regexp)
        self.my_each{|i| return true if arg.match(i.to_s)}
      else
        self.my_each{ |i| return true if i == args}
      end
    elsif block_given? #if block was given
      self.my_each{|i| true if yield(x)}
    else
      self.my_each{|i| true if i}
    end
    false
  end
  ...
  def my_none?(args=nil)
    if !args.nil? # if arguments or method parameters were given 
      if args.is_a?(Class)
        self.my_each{|i| return false if i.is_a?(args)}
      elsif args.is_a?(Regexp)
        self.my_each{|i| return false if arg.match(i.to_s)}
      else
        self.my_each{ |i| return false if i == args}
      end
    elsif block_given? #if block was given
      self.my_each{|i| false if yield(x)}
    else
      self.my_each{|i| false if i}
    end
    true
  end
  ...
  def my_count(args=nil)
    ...
    if block_given? #block was stated and args is nil
      ...
      self.my_each{|i| total += 1 if i == args}
    else
      return self.size
    end
    total
  ...
  def my_map(&block)
    ...
    mapped = []
    self.my_each { |i| mapped << block.call(i)}
    mapped
  end
...
[1,2,3,4,5].my_select {|x| puts x if x%2 ==0 }
